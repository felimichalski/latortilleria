$(document).ready( () => {
    ...
    // Toogle navbar on scroll
    const nav = $('nav');
    const logo = $(nav).find('.nav-brand img');
    ...
    $(window).on('scroll', () => {
        if($(this).scrollTop() > 5) {
            ...
            $(logo).attr('src', 'img/logos/logo2.png');
    ...
    $('.open-sidebar-btn').on('click', () => {
        $('.sidebar').addClass('show');
        $('.sidebar-overlay').css('visibility', 'visible');
        $('.sidebar-overlay').css('background', 'rgba(0, 0, 0, .7)');
    ...
    $(".subscribe-form").submit((e) => {
        ...
        .done((result) => {
            ...
            $(".subscribe-form .input-field input").val('').blur();
            setTimeout(() => {
        ...
        .fail((result) => {
            ...
            $(".error-msg").text(result.responseJSON.msg);
        })
    });
